...
func statuses_take_effect(m : Monster):
	for status : Status in m.current_statuses:
		var dmg : int = 0
		...
		if status.stacks_remaining <= 0: UI.debug(m.name + " wears off status " + status.name)
	
	m.current_statuses = m.current_statuses.filter(func(status : Status): return status.stacks_remaining > 0)
