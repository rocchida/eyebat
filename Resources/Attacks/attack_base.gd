...
func inflict_statuses(m: Monster):
	if !status_succeeded(attack_status.chance_to_hit):
		return
	if (m.current_statuses.has(attack_status)):
		for current_monster_status in m.current_statuses:
			if (current_monster_status == attack_status):
				current_monster_status.stacks_remaining = attack_status.stacks_on_hit
				return name + " is affected by " + attack_status.name + " again from attack"
	else: m.current_statuses.append(attack_status)
