...
func get_status_damage(ui : UI, m : Monster):
	...
	if (percent_dmg_current_hp_per_turn != 0):
		...
		ui.debug(m.name + " is damaged by " + name + ", it inflicts " + str(percent_dmg_current_hp_per_turn) + "% of current hp (" + str(m.health) + ") for a total of " + str(percent_dmg))
		dmg += percent_dmg
	if (percent_dmg_max_hp_per_turn != 0):
		var percent_dmg = int(percent_dmg_max_hp_per_turn * float(m.max_health))
		ui.debug(m.name + " is damaged by " + name + ", it inflicts " + str(percent_dmg_max_hp_per_turn) + "% of max hp (" + str(m.max_health) + ") for a total of " + str(percent_dmg))
		dmg += percent_dmg
	if (flat_dmg_per_turn != 0):
		var percent_dmg = int(flat_dmg_per_turn * float(m.max_health))
		ui.debug(m.name + " is damaged by " + name + ", it inflicts " + str(flat_dmg_per_turn) + " dmg")
		ui.debug("HP: " + str(m.health) + " -> NEW HP: " + str(m.health - flat_dmg_per_turn))
